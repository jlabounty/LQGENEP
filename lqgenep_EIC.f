...
C...FLAG=1 -> Second section: event generation  
      ...
      if(flag.eq.1)Then
       CALL PYEVNT
       print*,"The no. of event is",LQGPAR1(4)
       CALL PYHEPC(1)
...
**swadhin: ELECTRONIC TAU DECAY
*Here particle that are not decayed are called (except neutrinos) and their pT are added. The sum is called pT Miss.

       open(unit=30,file='DATA/20on325_miss.dat'
     >             ,status='new')
       open(unit=31, file='DATA/20on325_jet.dat'
     >       , status='new')
       open(unit=32, file='DATA/20on325_e.dat'
     >       , status='new')
       open(unit=33, file='DATA/20on325_tau.dat'
     >       , status='new')
       open(unit=34, file='DATA/20on325_nubare.dat'
     >       , status='new') 
     ...
     >        (K(J,2).EQ.15)) then      ! find tau and get it's line number
          ...
          if ((K(J,1).EQ.21).and.      !   
     >        (K(J,2).EQ.39)) then     ! Lepto quark of the process
           idlq=J                      ! Line number of Lq
          endif

          if ((K(J,3).EQ.idlq).and.
     >        (K(J,1).EQ.21).and.
     >        (K(J,2).NE.15)) then     
           idq=J                       ! Line number of out quark
           endif

          if  ((K(J,1).EQ.1).and.      ! find non decayable
     >        (K(J,2).EQ.-12).and.     ! electron anti-neutrino
     >        (K(J,3).EQ.idt)) then    ! see if it came from tau meaning select tau -> e + nubar_e + nu_tau
...
50        enddo
          ...
          do 55 l=1,N              ! produced quark
          if  (l.EQ.idq) then      ! find FIRST hadron jet (string) by 
                                   ! making sure it originated from the quark produced in e+ qi -> tau+ qj 
             ptj=PYP(l,10)
             pxj=P(l,1)
             pyj=P(l,2)
             phj=PYP(l,16)
...
c          write(8,10)lqgpar1(4),ptid,pxid,pyid,
c     >  pzid,phj,pxj,pyj,phe,pxe,pye
        write(30,10)lqgpar1(4),ptid,pxid,pyid,
     >   pzid
        write(31,11)lqgpar1(4),ptj,pxj,pyj,
     >   phj,ppj
        write(32,11)lqgpar1(4),pte,pxe,pye,
     >   phe,ppe
        write(33,11)lqgpar1(4),ptt,pxt,pyt,
     >   pht,ppt
        write(34,11)lqgpar1(4),ptne,pxne,pyne,
     >   phne,ppne



10      FORMAT(I8,4(1PE14.6))
11      FORMAT(I8,5(1PE14.6))
        endif
...
C..
       if(mod(LQGPAR1(4),100).eq.0)then
